...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
    ...
    @Autowired
    OmsOrderConfimMapper omsOrderConfimMapper;
    @Autowired
    ...
    private boolean prohasStock(List<ProductItem> productItemList) {
        for (ProductItem productItem : productItemList) {
            if (productItem.getStock() == null
                    || productItem.getStock() - productItem.getLockStock() - productItem.getQuantity() <= 0) {
                return false;
            }
        }
        return true;
    ...
    private ConfirmOrderResult.CalcAmount calcproductAmount(List<ProductItem> productItems) {
        ...
        for (ProductItem productItem : productItems) {
            ...
            promotionAmount = promotionAmount
                    .add(productItem.getReduceAmount().multiply(new BigDecimal(productItem.getQuantity())));
        }
        calcAmount.setTotalAmount(totalAmount);
        calcAmount.setPromotionAmount(promotionAmount);
        calcAmount.setPayAmount(totalAmount.subtract(promotionAmount));
